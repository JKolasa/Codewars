...
Given a set of n strings our function pos_average will calculate the average percentage
    of positions that are the same between the (n * (n-1)) / 2 sets of strings taken amongst the given 'n' strings.
...
public class PositionsAverage {
    public static double posAverage(String s) {
        ...
        int arrSize = splitted.length;
        int strLength = splitted[0].length();

        for(int i=0; i<arrSize; i++) {
            for(int j=i+1; j<arrSize; j++) {
                for(int k=0; k<strLength; k++) {
                    if(splitted[i].charAt(k) == splitted[j].charAt(k))
                        matched++;
                }
            }
        }
        return matched / (arrSize * strLength * (arrSize-1)/ 2) * 100;
    }
}
